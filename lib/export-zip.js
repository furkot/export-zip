...
function localFileHeader({ fname, len, crc, mod }) {
  return Uint8Array.from([
    0x50, 0x4b, 0x03, 0x04,    // FH signature
    0x0a, 0x00,                // min version
    0x00, 0x00,                // flag
    0x00, 0x00,                // store compression
...
function centralDirectoryFileHeader({ offset, fname, len, crc, mod }) {
  return Uint8Array.from([
    0x50, 0x4b, 0x01, 0x02,     // CE signature
    0x1e, 0x03,                 // version
    0x0a, 0x00,                 // min version
    0x00, 0x00,                 // flag
    0x00, 0x00,                 // store compression
    ...
    0x00, 0x00,                 // attrs internal
    0x00, 0x00, 0xb4, 0x81,     // attrs external - coresponds to 0100664
    ...uint32(offset)
  ]);
}
